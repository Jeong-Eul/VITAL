...
#         self.final_norm = nn.BatchNorm1d(N*d_ff + d_ff)
#         self.final_dropout = nn.Dropout(head_dropout)
#         self.activation_fn = nn.GELU()
#         self.fc_final = nn.Linear(N * d_ff + d_ff, target_window)
...
class Ehrtimellm(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # P12 variable information
        self.vital_index = [8, 14, 21, 22, 30] # 순서대로 DBP, HR, NDBP, NMAP, SBP |  NSBP, MAP, RR은 EDA 결과 lab 만큼 희소해서 제외
        self.lab_index = list(set(np.arange(self.enc_in)) - set(self.vital_index))
        
        # P19 variable information
        
        # self.vital_index = [0, 1, 3, 4, 5]
        # self.lab_index = list(set(np.arange(self.enc_in)) - set(self.vital_index))

        self.dropout = nn.Dropout(configs.dropout)
        
        # static
        self.emb = nn.Linear(self.d_static, self.d_ff) # modified
